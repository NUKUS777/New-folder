...
const newElement= users.map(function finder(userData) {
    return ` <div class="col-lg-3 col-md-4 col-sm-6 m-2">
    <div class="card p-3  border-primary-subtle bg-primary-subtle" style="width: 18rem;">
        <img src="${userData.avatar_url}" class="card-img-top rounded-circle" alt="Avatar">
        <div class="card-body">
          <h5 class="card-title text-info">Id: ${userData.id}</h5>
           <h4 class="card-text text-info">Login: ${userData.login}</h4>
          <a href="#" class="card-link">${userData.url}</a>
        </div>
      </div>
  </div>`
}
